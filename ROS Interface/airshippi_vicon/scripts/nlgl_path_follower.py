...
import time
import sys
from math import pi, sin, atan2, sqrt
...
class CarrotPathFollower(object):
	...
	# Get point on path at a distance L 
	L_index = self.path.get_point_at_distance(current_pos, L, reverse=False)
	self.vtp = self.path.get_point(L_index)
	...
	pwm_shift = P*self.speed**(speed_exp)*sin(self.error)/L
	...
	# Set rate
	    ...
	    rospy.loginfo("\nError: %s, Speed: %s", self.error, self.speed)
            rate.sleep()
